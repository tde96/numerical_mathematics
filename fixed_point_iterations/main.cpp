...
#include <fstream>
#include <string>
//#includen <conio.h>       //für Windows
//#include <iomanip>        //für Windows

#define tz ','                              // Trennzeichen (Konstante)
#define CSVdatei "fi_werte.csv"             // Dateiname  (Konstante)
...
int main() {
    ...
    // Ausgabe Titel
    cout << "Numerische Mathematik Programmieraufgabe 1: Fixpunktiteration\n\n";
    ...
    cout << left << setw(15) << "k" << setw(15) << "xn" << "|x_k - x_k-1|" << endl << endl;
    
    // Erster Eintrag Tabelle
    csv << left << setw(15) << 0 << setw(15) << fixed << setprecision(10) << x0 << " " << '\n';           //CSV
    cout << left << setw(15) << 0 << setw(15) << fixed << setprecision(10) << x0 << " " << endl;          //Console
    ...
    if (!csv.fail()) { // Ist KEIN Fehler beim Oeffnen aufgetreten?
        do {
            ...
            csv << left << setw(15) << k << setw(15) << fixed << setprecision(10) << x0 << abs(x0-x_kminus1) << '\n';
                             
            // Ausgabe in Konsole
            cout << left << setw(15) << k << setw(15) << fixed << setprecision(10) << x0 << abs(x0-x_kminus1) << endl;
            
            // Zähler erhöhen
            k = k + 1;
            
        } while (abs(x0-x_kminus1) > 0.00000001);           // Breche die Iteration ab wenn die Genauigkeit erreicht ist
    }
...
// x - f(x)/f'(x) für das Newton Verfahren:
//double fi_funktion(double x) {return (2*x-(x+5)*log(x+5)+5)/(x+6); }
...
//double fi_funktion(double x) { return x - (x*x*x*x*x*x-x-1)/(6*x*x*x*x*x-1); }
//double fi_funktion(double x) {return x*x*x + 0.3;}

//Uebungsblatt 3 Aufg. 9
//double fi_funktion(double x) {return 2*sin(x);}
//double fi_funktion(double x) {return x-(2*sin(x)-x)/(2*cos(x)-1);}
...
Old Version - (keine Genauigkeit, nur 20 Iterationen):
 for (i = 1; i <= 21; i++) {
    ...
    // Speichere x0 und berechne x0 neu
     ...
     // Werte in CSV eintragen
     csv << left << setw(15) << n << setw(15) << fixed << setprecision(10) << x0 << '\n';
                      
     // Ausgabe in Konsole
     cout << left << setw(15) << n << setw(15) << fixed << setprecision(10) << x0 << abs(x0-x_kminus1) << endl;
